...
router.post('/:id/resolve', async (req, res) => {
  ...
  try {
    await db.execute('UPDATE support_tickets SET status = ? WHERE id = ?', ['Resolved', ticketId]);
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    await db.execute('UPDATE support_tickets SET is_deleted = 1 WHERE id = ?', [ticketId]);
