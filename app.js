  ...
  helmet({
    contentSecurityPolicy: {
      ...
      directives: {
        ...
        objectSrc: ["'none'"],
        baseUri: ["'self'"],
        ...(PROD ? { 'upgrade-insecure-requests': [] } : {}), // ✅ hyphenated and only in prod
      },
    },
    ...
    noSniff: true,
    // xssFilter: true, // ❌ remove (deprecated)
  })(req, res, next);
});
...
// ---------- 6. Static files ----------
// (Move this block AFTER the routers; and add no-cache for HTML)
app.use(
  express.static(path.join(__dirname, 'public'), {
    ...
    setHeaders(res, filePath) {
      if (/\.html?$/i.test(filePath)) {
        res.setHeader('Cache-Control', 'no-cache'); // ✅ ensure fresh HTML
        return;
      }
...
app.use(['/api/plans', '/plans'], plansRouter);
...
app.use('/api/reviews/analytics', require('./routes/reviewAnalytics')); // ✅ put analytics under a subpath
...
// DEBUG: list mounted routes (enable only in non-prod or for debugging)
if (process.env.ROUTES_DEBUG === '1') {
  app.get('/debug/routes', (req, res) => {
    ...
    app._router.stack.forEach((layer) => {
      if (layer.route && layer.route.path) {
        ...
        const base = layer.regexp?.fast_star ? '*' : (layer.regexp?.fast_slash ? '/' : (layer.regexp?.toString() || ''));
        layer.handle.stack.forEach((r) => {
          if (r.route?.path) {
            const methods = Object.keys(r.route.methods).join(',').toUpperCase();
            list.push({ path: (layer.regexp?.prefix || '') + r.route.path, methods });
          }
        });
      }
    });
